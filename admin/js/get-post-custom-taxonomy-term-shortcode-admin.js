(function($) {
    $(window).load(function() {
        ...
        $(document).on('change', '.post_per_page_value', function() {
            $('#custom_term_loading').show();
            $('#gtpagination').css("display", "block");
            var post_per_page_value = $(this).val();
            var data = {
                'action': 'gt_pagination',
                'post_per_page': post_per_page_value
            };
            jQuery.post(ajaxurl, data, function(response) {
                if (response) {
                    $('#custom_term_loading').hide();
        ...
        $(document).on('change', '.select_textonomy', function() {
            ...
            //alert(selected_term);
            if (selected_term != "Select Taxonomy") {
        ...
        $(document).on('click', '#submit.get_shortcode', function() {
            var select_post_data = $('.shortcode_main').val();
            if (select_post_data != 'select-post-type') {
                var select_term_data = $('.select_term option:selected').text();
                var select_texonomy_data = $('.select_textonomy').val();
                var post_type = "post_listing_data post_name='" + select_post_data + "'"
                var post_per_page=$('.post_per_page_value').val();
                
                var sorting = $('.form-table .sort-option input[name=sorting]:checked').attr("value");
                var display_view_data = $('.form-table .color-option input[name=post_view]:checked').attr("value");
                if (select_texonomy_data != 'Select Taxonomy' && select_texonomy_data != 'undefined') {
                ...
                if (post_per_page == '') {
                    var post_per_page_limit = "";
                    
                } else {
                    var post_per_page_limit = " post_per_page_list='" + post_per_page + "'";
                ...
                var final_data = "[" + post_type + post_per_page_limit + sorting + taxonomy + term + display_view_type + "]";
